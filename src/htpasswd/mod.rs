...
static BCRYPT_ID_UTF8: &str = "$2a$";
static BCRYPT_ID_OPENBSD: &str = "$2b$";
...
impl<'a> Hash<'a> {
    pub fn check<S: AsRef<str>>(&self, password: S) -> bool {
        ...
        match self {
            ...
            Hash::SHA1(hash) => {
                ...
                STANDARD.encode(&hashed).as_str() == *hash
            }
    ...
    pub fn parse(hash: &'a str) -> Hash<'a> {
        if hash.starts_with(APR1_ID) {
            ...
            || hash.starts_with(BCRYPT_ID_OPENBSD)
            || hash.starts_with(BCRYPT_ID_UTF8)
        {
